...
def run_scenario_analysis_vectorized(teams, tonight_games, num_simulations=100000, real_schedule=None):
    ...
    # best/worst per team — forced simulation approach.
    # For each team, we force all favorable (or unfavorable) outcomes simultaneously
    # and run a fresh simulation. This gives exact compound probabilities.
    best_worst = {}
    ...
    # Build the future schedule (tomorrow onwards) — tonight's games come via forced_outcomes
    if real_schedule:
        tonight_pairs_set = {(g["homeTeamAbbrev"], g["awayTeamAbbrev"]) for g in valid_games}
        future_schedule = [
            (h, a) for h, a in real_schedule
            if h in abbrev_to_idx and a in abbrev_to_idx
            and (h, a) not in tonight_pairs_set
        ]
    else:
    ...
    for abbrev in all_abbrevs:
        ...
        # Get games that affect this team's playoff chances (delta >= 0.1%)
        relevant_games = []
        for scenario in team_scenarios.get(abbrev, []):
            game_id = scenario.get("game_id")
            home_delta = scenario.get("home_delta_reg") or scenario.get("home_delta")
            away_delta = scenario.get("away_delta_reg") or scenario.get("away_delta")
            max_delta = max(abs(home_delta) if home_delta is not None else 0,
                           abs(away_delta) if away_delta is not None else 0)
            # Find matching game from valid_games (include all, no delta threshold for forced sim)
            for g in valid_games:
                if str(g.get("gameId", f"{g['homeTeamAbbrev']}_{g['awayTeamAbbrev']}")) == str(game_id):
        ...
        # Run forced simulations
        # For best case: all favorable outcomes
        if fav_outcomes:
            fav_result = run_simulations_with_multiple_forced(
                teams, fav_outcomes, num_simulations=100000, real_schedule=future_schedule
            )
            best_pct = fav_result.get(abbrev, {}).get("playoff_pct", baseline)
            if abbrev == "WSH":
                print(f"[WSH DEBUG] fav: {fav_outcomes} => {best_pct}%, baseline={baseline}")
        else:
            best_pct = baseline
        
        # For worst case: all unfavorable outcomes
        if unfav_outcomes:
            unfav_result = run_simulations_with_multiple_forced(
                teams, unfav_outcomes, num_simulations=100000, real_schedule=future_schedule
            )
            worst_pct = unfav_result.get(abbrev, {}).get("playoff_pct", baseline)
            if abbrev == "WSH":
                print(f"[WSH DEBUG] unfav: {unfav_outcomes} => {worst_pct}%, baseline={baseline}")
        else:
            worst_pct = baseline
    ...
    for offset in range(0, num_simulations, batch_size):
        ...
        for conf_idx, divs in enumerate(conf_div_structure):
            ...
            if playoff_batch is not None:
                ...
                for s in range(b):
                    in_mask = conf_playoff[s]
                    out_mask = ~in_mask

                    in_idxs  = np.where(in_mask)[0]
                    out_idxs = np.where(out_mask)[0]

                    # Build global→local mapping for this conference
                    global_to_local = {conf_team_idxs[i]: i for i in range(len(conf_team_idxs))}

                    # Classify each playoff team by their position in their division
                    div_winners = []       # 1st in division
                    div_seconds = []       # 2nd in division
                    div_thirds = []        # 3rd in division
                    already_seeded = set() # global idxs already assigned seeds 1-8

                    for div_global_idxs in divs:
                        # Convert global indices to local indices
                        div_local_idxs = [global_to_local[g] for g in div_global_idxs if g in global_to_local]
                        # Get playoff teams in this division, ranked by points
                        div_playoff_local = [li for li in div_local_idxs if in_mask[li]]
                        if not div_playoff_local:
                            continue
                        # Sort by points descending
                        div_playoff_local.sort(key=lambda li: -conf_comp[s, li])

                        if len(div_playoff_local) >= 1:
                            div_winners.append(div_playoff_local[0])
                        if len(div_playoff_local) >= 2:
                            div_seconds.append(div_playoff_local[1])
                        if len(div_playoff_local) >= 3:
                            div_thirds.append(div_playoff_local[2])

                    # Seeds 1-2: Division winners, ranked by points
                    if div_winners:
                        dw_pts = [(li, conf_comp[s, li]) for li in div_winners]
                        dw_ranked = [x[0] for x in sorted(dw_pts, key=lambda x: -x[1])]
                        for rank, local_idx in enumerate(dw_ranked[:2]):
                            global_idx = conf_team_idxs[local_idx]
                            seed_counts[global_idx, rank + 1] += 1
                            already_seeded.add(global_idx)

                    # Seeds 3-4: 2nd in each division, ranked by points
                    if div_seconds:
                        ds_pts = [(li, conf_comp[s, li]) for li in div_seconds]
                        ds_ranked = [x[0] for x in sorted(ds_pts, key=lambda x: -x[1])]
                        for rank, local_idx in enumerate(ds_ranked[:2]):
                            global_idx = conf_team_idxs[local_idx]
                            seed_counts[global_idx, rank + 3] += 1
                            already_seeded.add(global_idx)

                    # Seeds 5-6: 3rd in each division, ranked by points
                    if div_thirds:
                        dt_pts = [(li, conf_comp[s, li]) for li in div_thirds]
                        dt_ranked = [x[0] for x in sorted(dt_pts, key=lambda x: -x[1])]
                        for rank, local_idx in enumerate(dt_ranked[:2]):
                            global_idx = conf_team_idxs[local_idx]
                            seed_counts[global_idx, rank + 5] += 1
                            already_seeded.add(global_idx)

                    # Seeds 7-8: Wildcards — best remaining non-division-winners by points
                    remaining_local = [li for li in in_idxs if conf_team_idxs[li] not in already_seeded]
                    if remaining_local:
                        rem_comp = [(li, conf_comp[s, li]) for li in remaining_local]
                        rem_ranked = [x[0] for x in sorted(rem_comp, key=lambda x: -x[1])]
                        for rank, local_idx in enumerate(rem_ranked[:2]):
                            global_idx = conf_team_idxs[local_idx]
                            seed_counts[global_idx, rank + 7] += 1
                            already_seeded.add(global_idx)

                    # Seeds 9-16: Non-playoff teams
                    if len(out_idxs) > 0:
